...
def delete_snapshot_in_repository(client_config, repository, snapshot):
  ...
  elastic_connection = es.build_es_connection(client_config)
  try:
    delete_status = elastic_connection.snapshot.delete(repository, snapshot=snapshot)
    elastic_connection.close()
    ...
    elastic_connection.close()
    print("Operation failed - Delete snapshot " + snapshot + " from " + repository)
    raise Exception(e)
  return False
...
def take_snapshot(client_config, repository, snapshot, body):
  ...
  try:
    ...
    if DEBUG_ENABLED == "1":
      ...
      print("Repository is " + repository + "| snapshot is " + snapshot_name + " | body is:")
      print(json.dumps(body))
    elastic_connection = es.build_es_connection(client_config)
    backup_job = elastic_connection.snapshot.create(repository, snapshot_name, body, wait_for_completion=False, request_timeout=30)
    elastic_connection.close()
  ...
  for index in indices:
    ...
    try:
      ...
      for index in result['aggregations']['indices']['buckets']:
        ...
        index_date = dateparser.parse(index['1']['value_as_string']).replace(tzinfo=None)
...
def take_snapshot_per_policies(client_config, job, backup_policy, repository, include_special=False):
  ...
  if job == 'global':
    ...
    if 'limit_age' in backup_policy:
      indices = get_indices_within_limit_age(client_config, indices, backup_policy['limit_age'])
      index = modify_indices_to_string(indices) + "," + modify_indices_to_string(special_indices_to_backup, wildcard=True)
    else:
      index = '*'
  # .kibana backs up global state and .kibana indices
  elif job == '.kibana' or job == "special":
    index = modify_indices_to_string(special_indices_to_backup, wildcard=True)
  else:
    if 'limit_age' in backup_policy:
      backup_policy = { "retention": 1, "limit_age": 1}
      indices = get_indices_within_limit_age(client_config, [job], backup_policy['limit_age'])
      if include_special:
        index = modify_indices_to_string(indices) + "," + modify_indices_to_string(special_indices_to_backup, wildcard=True)
      else:
  ...
  if backup_policy['retention'] != 0:
    ...
    if backup_job:
      ...
      # Should not hit this point unless retry failed for an hour
      message = "Backup take snapshot failed for " + client_config['client_name'] + " for " + job + " in repository " + repository
      print(message)
      send_notification(client_config, "backup", "Failed", message, teams=settings['backup']['ms-teams'], jira=settings['backup']['jira'])
            ...
            for job in backup_policy:
              if 'retention' in backup_policy[job]:
                if backup_policy[job]['retention'] != 0:
                  print(f"Processing backups for repository {repository} with job of {job}")
                  apply_backup_retention_policies(client_config, job, backup_policy[job]['retention'], repository)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--notification", default="True", type=str, help="Set to enable notifications")
    parser.add_argument("--debug", default="True", type=str, help="Set enable debug")
